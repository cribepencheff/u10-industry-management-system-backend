...
async function seedDatabase() {
  ...
  const manufacturers = Array.from({ length: 8 }).map(() => ({
    ...
    address: faker.location.streetAddress() || "Unknown Address",
  ...
  const products = Array.from({ length: 20 }).map((_, index) => {
    let amountInStock = Number(faker.string.numeric({ length: 2 }));

    // UPDATED: 8 products with stock under 10
    if (index < 8) {
      amountInStock = faker.number.int({ min: 1, max: 9 });

      // UPDATED: 4 of them are critical stock (1–2)
      if (index < 4) {
        amountInStock = faker.number.int({ min: 0, max: 2 });
      }
    }
